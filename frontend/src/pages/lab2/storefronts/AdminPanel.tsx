...
export default function AdminPanel({ variant, data }: AdminPanelProps) {
  ...
  const handleDeleteUser = async (userId: number) => {
    ...
    try {
      ...
      const res = await axios.delete(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/lab2/${labModule}/${actualVariant}/admin/users/${userId}`, { withCredentials: true });
      if (res.data.success) {
