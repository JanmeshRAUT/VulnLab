...
function Lab1Sub1Content() {
  ...
  useEffect(() => {
    if (instanceId) {
      ...
      axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/lab1/1/files`, { 
    ...
    <div className="w-full min-h-screen bg-[#f4f7f9] flex font-sans text-slate-800 relative overflow-hidden">
      ...
      <main className="flex-1 h-screen overflow-y-auto z-10 flex flex-col relative">
        ...
        <header className="md:hidden flex items-center justify-between mb-8 bg-white/80 backdrop-blur-md p-4 rounded-2xl border border-slate-200/60 shadow-sm">
           ...
           <button className="p-2 bg-slate-100 rounded-lg"><MoreVertical size={20}/></button>
        </header>
        ...
        <div className="hidden md:flex justify-between items-center mb-10">
          <div className="text-sm text-slate-500 font-medium flex items-center gap-2">
            ...
            <span className="text-slate-800 font-bold px-2 py-1 bg-white rounded-md border border-slate-200 shadow-sm">My Documents</span>
          </div>
        ...
        {/* Welcome Section */}
        <div className="mb-10">
          <h1 className="text-4xl font-black text-slate-900 mb-2 tracking-tight">Your Documents</h1>
          <p className="text-lg text-slate-500 font-medium max-w-2xl">
            Secure cloud storage for all your important files. Manage, share, and collaborate effortlessly.
          </p>
        </div>
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100/50 rounded-2xl p-6 flex gap-4 mb-10 shadow-sm">
          ...
          <div>
            <h3 className="font-bold text-blue-900 mb-1 text-lg">Investigate the Endpoint</h3>
            <p className="text-blue-700/80 font-medium">
              Your documents are securely stored in our cloud infrastructure. Can you traverse to an unintended directory using the download feature?
            </p>
          </div>
        </div>
        ...
        <div className="flex justify-between items-end border-b border-slate-200/60 pb-4 mb-6">
          ...
          <div className="text-slate-500 font-medium text-sm bg-white px-3 py-1 rounded-full shadow-sm border border-slate-200">
            <span className="font-bold text-slate-700">{files.length}</span> items
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-20">
            {files.map((file, idx) => {
              const isRecent = file.includes('2024');
              return (
                <div key={idx} className="bg-white/80 backdrop-blur border border-slate-200/60 rounded-2xl p-5 shadow-sm hover:shadow-xl hover:-translate-y-1 hover:border-blue-300/60 transition-all duration-300 flex flex-col group relative overflow-hidden">
                  ...
                  <h3 className="font-bold text-slate-800 truncate text-base mb-1 group-hover:text-blue-700 transition-colors" title={file}>
                    {file}
                  </h3>
                  
                  <div className="flex items-center gap-2 text-xs text-slate-500 font-medium mb-6">
                    <span className={`w-2 h-2 rounded-full shadow-sm ${isRecent ? 'bg-green-500' : 'bg-slate-300'}`}></span>
                    {isRecent ? 'Modified Today' : 'Archived'}
                  </div>
                  
                  <div className="mt-auto flex gap-2 pt-4 border-t border-slate-100/80">
                    <a 
                      href={`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/lab1/1/download?file=${file}`}
                      target="_blank"
                      rel="noreferrer"
