...
const FALLBACK_ARCHIVE_FILES: ArchiveFile[] = [
  { file: 'Invoice_2024_001.pdf', archivedOn: 'Archived 2 days ago', category: 'Invoices' },
  { file: 'Meeting_Minutes_Jan.txt', archivedOn: 'Archived last week', category: 'Notes' },
  { file: 'Vendor_List.csv', archivedOn: 'Archived 2 weeks ago', category: 'Operations' },
  { file: 'Marketing_Assets.zip', archivedOn: 'Archived this month', category: 'Assets' }
];
...
function ArchivesContent() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        const generatedArchive = res.data.slice(-6).map((file, idx) => ({
          file,
          archivedOn: ['Archived today', 'Archived yesterday', 'Archived 2 days ago', 'Archived 4 days ago', 'Archived last week', 'Archived last week'][idx] || 'Archived recently',
          category: ['Invoices', 'Reports', 'Contracts', 'Notes', 'Assets', 'General'][idx] || 'General'
        }));

        setArchiveFiles(generatedArchive.length ? generatedArchive : FALLBACK_ARCHIVE_FILES);
        setLoading(false);
      })
      .catch(() => {
        setArchiveFiles(FALLBACK_ARCHIVE_FILES);
        setLoading(false);
      });
  }, [instanceId]);
  ...
  const archiveCountLabel = useMemo(() => `${archiveFiles.length} archived items`, [archiveFiles.length]);
        ...
        <div className="w-full max-w-6xl mx-auto flex flex-col p-6 lg:p-10 pb-24">
          ...
          <div className="hidden md:flex justify-between items-center mb-10">
            <div className="text-sm text-slate-500 font-medium flex items-center gap-2">
              ...
              <span className="text-slate-300">/</span>
              <span className="text-slate-800 font-bold px-2 py-1 bg-white rounded-md border border-slate-200 shadow-sm">Archives</span>
            </div>
          ...
          <div className="mb-8 flex flex-col gap-3">
            ...
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">Archives</h1>
            <p className="text-lg text-slate-500 font-medium max-w-2xl">
              Previously active files moved to archive for long-term retention.
            </p>
          </div>
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100/50 rounded-2xl p-6 flex gap-4 mb-10 shadow-sm">
            ...
            <div>
              <h3 className="font-bold text-blue-900 mb-1 text-lg">Archived Storage</h3>
              <p className="text-blue-700/80 font-medium">
                Archived files remain accessible through the same document download endpoint.
              </p>
            </div>
          </div>

          <div className="flex justify-between items-end border-b border-slate-200/60 pb-4 mb-6">
            <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">Archived Documents</h2>
            <div className="text-slate-500 font-medium text-sm bg-white px-3 py-1 rounded-full shadow-sm border border-slate-200">
              <span className="font-bold text-slate-700">{archiveCountLabel}</span>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-20">
              {archiveFiles.map((item, idx) => (
                <div key={`${item.file}-${idx}`} className="bg-white/80 backdrop-blur border border-slate-200/60 rounded-2xl p-5 shadow-sm hover:shadow-xl hover:-translate-y-1 hover:border-blue-300/60 transition-all duration-300 flex flex-col group relative overflow-hidden">
                  ...
                  <div className="text-xs text-slate-500 font-medium mb-2">{item.archivedOn}</div>
                  <div className="text-xs text-slate-600 font-semibold mb-5">Category: {item.category}</div>
...
export default function Lab1Archives() {
