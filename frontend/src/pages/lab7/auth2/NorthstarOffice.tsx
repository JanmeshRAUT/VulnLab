...
export default function NorthstarOffice() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setStatus('loading');
    try {
      const res = await axios.post(`${API_BASE}/api/lab7/2/a/login`, 
        { username, password },
        { 
  ...
  const fetchUsers = async () => {
    ...
    try {
      const res = await axios.get(`${API_BASE}/api/lab7/2/a/users`, {
        withCredentials: true,
        headers: { 'X-Variant-Session-ID': instanceId }
      });
  ...
  const handleDeleteUser = async (targetUsername: string) => {
    if (!instanceId) return;
    try {
      const res = await axios.delete(`${API_BASE}/api/lab7/2/a/users/${targetUsername}`, {
        withCredentials: true,
        headers: { 'X-Variant-Session-ID': instanceId }
      });
      if (res.data.success) {
