...
export default function TechStore({ instanceId }: Props) {
  ...
  return (
    <div className="w-full min-h-screen bg-slate-50 text-slate-900 font-sans flex flex-col relative selection:bg-blue-500/20">
      {/* Header */}
      <header className="bg-white/80 backdrop-blur-2xl border-b border-slate-200 px-8 py-4 flex justify-between items-center sticky top-0 z-50">
        <button onClick={() => setView('home')} className="flex items-center gap-3 font-black text-2xl tracking-tight text-slate-900 group">
          <div className="p-2 bg-blue-50 rounded-xl group-hover:scale-105 transition-transform border border-blue-100">
            <MonitorSmartphone size={24} className="text-blue-600" strokeWidth={2.5} />
          </div>
          TechStore
        </button>
        <div className="flex items-center gap-8">
          <nav className="flex gap-8 text-sm font-bold hidden md:flex uppercase tracking-widest text-slate-500">
            <span className="cursor-pointer hover:text-blue-600 transition-colors">Products</span>
            <span className="cursor-pointer hover:text-blue-600 transition-colors">Solutions</span>
          ...
          <div className="relative flex items-center">
            <button 
              ...
              className="relative p-2.5 rounded-full hover:bg-blue-50 text-slate-400 hover:text-blue-600 transition-colors flex items-center justify-center group mr-4 border border-transparent hover:border-blue-200"
              title="Webmail Inbox"
            >
              <Mail size={20} className="group-hover:scale-110 transition-transform" />
              {emails.length > 0 && <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse border-2 border-white"></span>}
            </button>
            {showEmail && (
              <div className="absolute right-0 top-full mt-4 w-96 bg-white/95 backdrop-blur-2xl border border-slate-200 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300 text-left z-50">
                <div className="bg-slate-50 p-4 border-b border-slate-200 flex justify-between items-center text-slate-500">
                  <span className="font-bold text-xs uppercase tracking-widest flex items-center gap-2 text-blue-600"><Mail size={14} /> exploit-server.local</span>
                ...
                <div className="p-4 max-h-[400px] overflow-y-auto space-y-3">
                  {emails.length === 0 ? (
                    ...
                    emails.map((e, i) => (
                      <div key={i} className="bg-white p-4 rounded-xl border border-slate-200 text-sm hover:border-blue-300 transition-colors shadow-sm">
            ...
            {isLoggedIn ? (
              <button onClick={() => {
                ...
                setIsLoggedIn(false);
                setView('login');
              }} className="px-6 py-2.5 rounded-full text-sm font-bold shadow-[0_4px_15px_rgba(59,130,246,0.2)] transition-all hover:scale-105 active:scale-95 bg-blue-600 hover:bg-blue-700 text-white flex items-center gap-2">
                Sign Out
              </button>
            ) : (
              <button onClick={() => setView('login')} className="px-6 py-2.5 rounded-full text-sm font-bold shadow-[0_4px_15px_rgba(59,130,246,0.2)] transition-all hover:scale-105 active:scale-95 bg-blue-600 hover:bg-blue-700 text-white flex items-center gap-2">
                Sign In
              </button>
            )}
          </div>
      ...
      <main className="flex-1 flex flex-col relative">
        <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-blue-100/50 rounded-full blur-[150px] pointer-events-none"></div>
        <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-cyan-100/50 rounded-full blur-[150px] pointer-events-none"></div>
      ...
      <section className="relative w-full pt-20 pb-32 px-8 overflow-hidden flex-1 flex items-center">
        <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10">
          <div className="flex flex-col items-start animate-in fade-in slide-in-from-left-8 duration-1000">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold mb-8 uppercase tracking-widest bg-blue-50 text-blue-600 border border-blue-200 shadow-sm">
            ...
            <p className="text-xl mb-12 leading-relaxed max-w-lg font-medium text-slate-600">
              Experience seamless, military-grade security without compromising on performance. Join millions of users who trust TechStore for their daily operations.
            </p>
            
            <div className="flex flex-wrap gap-4 w-full sm:w-auto">
              <button onClick={() => setView('login')} className="px-8 py-4 rounded-xl text-lg font-black shadow-[0_8px_25px_rgba(59,130,246,0.3)] transition-all hover:scale-105 hover:-translate-y-1 flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white">
                Sign In to Dashboard <ChevronRight size={20} />
              </button>
              <button className="px-8 py-4 rounded-xl text-lg font-bold transition-all border border-slate-200 text-slate-700 hover:bg-slate-50 bg-white shadow-sm flex items-center justify-center">
                Learn More
              </button>
            </div>
            
            <div className="mt-10 flex items-center gap-8 text-sm font-bold text-slate-500 uppercase tracking-widest">
              <div className="flex items-center gap-2"><CheckCircle2 size={16} className="text-blue-600"/> No credit card required</div>
              <div className="flex items-center gap-2"><CheckCircle2 size={16} className="text-blue-600"/> 14-day free trial</div>
            </div>
          </div>
          
          <div className="relative animate-in fade-in zoom-in-95 duration-1000 delay-200 hidden lg:block">
            <div className="absolute inset-0 bg-gradient-to-tr from-blue-100 to-cyan-50 rounded-3xl blur-[80px] opacity-50 animate-pulse"></div>
            <div className="relative rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.05)] overflow-hidden border border-slate-200 backdrop-blur-xl p-8 min-h-[450px] bg-white/80">
              ...
              <div className="space-y-6">
                ...
                <div className="h-40 w-full rounded-2xl animate-pulse bg-blue-50/50 border border-blue-100"></div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="p-10 rounded-[2rem] border border-slate-200 transition-all duration-500 hover:-translate-y-2 shadow-sm bg-white hover:border-blue-300 hover:shadow-[0_10px_30px_rgba(59,130,246,0.1)] group">
                <div className="w-20 h-20 rounded-2xl flex items-center justify-center mb-8 shadow-sm bg-blue-50 text-blue-600 border border-blue-100 group-hover:scale-110 transition-transform duration-500">
                  {f.icon}
                </div>
    ...
    <div className="flex-1 flex items-center justify-center p-8 z-10">
      <div className="w-full max-w-md p-10 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] bg-white/90 backdrop-blur-2xl border border-slate-200 animate-in fade-in zoom-in-95 duration-500">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 font-medium">Sign in to your TechStore account</p>
        </div>
        <form onSubmit={handleLogin} className="space-y-6">
          <div>
            ...
            <input type="text" value={username} onChange={e => setUsername(e.target.value)} required
              className="w-full px-5 py-4 rounded-xl outline-none border border-slate-200 transition-all focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-slate-50 text-slate-900 font-medium placeholder-slate-400" placeholder="Enter username" />
          </div>
          <div>
            <label className="block text-xs font-bold mb-2 text-slate-500 uppercase tracking-widest">Password</label>
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} required
              className="w-full px-5 py-4 rounded-xl outline-none border border-slate-200 transition-all focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-slate-50 text-slate-900 font-medium placeholder-slate-400" placeholder="••••••••" />
          </div>
          <button type="submit" className="w-full py-4 mt-6 rounded-xl font-black text-white shadow-[0_8px_20px_rgba(59,130,246,0.25)] transition-transform active:scale-[0.98] flex justify-center items-center gap-2 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-lg">
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-8 z-10">
      <div className="w-full max-w-md p-12 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] text-center bg-white/90 backdrop-blur-2xl border border-slate-200 animate-in fade-in zoom-in-95 duration-500">
        <div className="inline-flex justify-center items-center w-20 h-20 rounded-full mb-8 bg-blue-50 text-blue-600 border border-blue-100 shadow-sm">
          <ShieldCheck size={40} />
        </div>
        ...
        <form onSubmit={handleVerify} className="space-y-6">
          <input type="text" maxLength={4} placeholder="0000" value={code} onChange={e => setCode(e.target.value)} required
            className="w-full text-center text-5xl tracking-[0.5em] font-mono py-6 rounded-2xl outline-none border border-slate-200 transition-all focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-slate-50 text-slate-900 placeholder-slate-300" />
          <button type="submit" className="w-full py-4 mt-4 rounded-xl font-black text-white shadow-[0_8px_20px_rgba(59,130,246,0.25)] transition-transform active:scale-[0.98] flex justify-center items-center gap-2 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-lg">
...
function AccountPage({ variantId, instanceId, setView, setIsLoggedIn }: any) {
  ...
  if (!profile && !error) return <div className="flex-1 flex justify-center items-center font-bold text-slate-500 text-xl animate-pulse">Loading Dashboard...</div>;
    ...
    <div className="flex-1 p-8 md:p-12 max-w-5xl mx-auto w-full relative z-10">
      ...
      {profile && (
        <div className="p-10 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border bg-white/80 border-slate-200 backdrop-blur-xl relative overflow-hidden">
          <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-blue-50 rounded-full blur-[80px] pointer-events-none"></div>
          
          <div className="flex items-center gap-8 mb-10 pb-10 border-b border-slate-100 relative z-10">
            <div className="w-24 h-24 bg-gradient-to-tr from-blue-600 to-cyan-500 rounded-2xl flex items-center justify-center text-white shadow-[0_8px_25px_rgba(59,130,246,0.25)]">
              <User size={48} />
            </div>
            <div>
              <h2 className="text-4xl font-black text-slate-900 mb-2">{profile.username}</h2>
              <div className="text-lg font-medium text-slate-500 flex items-center gap-2"><Mail size={18}/> {profile.email}</div>
            </div>
          </div>
